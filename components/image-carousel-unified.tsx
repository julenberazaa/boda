...
export default function ImageCarousel({
  ...
  // REMOVED: windowWidth tracking - Carousels ahora usan altura fija de 384px sin adaptación
  ...
  return (
    <div
      ref={carouselRef}
      {...rest}
      className={`relative w-full ${calibrationMode ? 'cursor-crosshair' : 'cursor-pointer'}${className ? ` ${className}` : ''}`}
      style={{ height: '384px', ...rest.style }}
    ...
    >
      ...
      {/* Frame overlay - escala fija del frameConfig (sin adaptación responsive) */}
      {frameSrc && (
        <img
          src={frameSrc}
          alt=""
          style={{
            ...
            width: '100%',
            height: '100%',
            transform: `translate(-50%, -50%) scale(${frameConfig?.scaleX ?? 1.2}, ${frameConfig?.scaleY ?? 1.2})`,
            objectFit: 'contain',
            zIndex: 30,
            pointerEvents: 'none'
          }}
